...
这个自定义连接器允许 Power BI 直接连接到 Wolai 数据表格并导入数据。
...
*   **全量数据导入**: 支持获取 Wolai 数据表的所有记录到PowerBI方便和多个数据源的数据综合进行分析，也方便在PowerBI内通过URL链接打开wolai编辑对应的表格数据。
*   **自动分页**: 针对大型表格（超过 200 行）实现了自动游标分页 (`start_cursor`)，解决 502 网关超时问题。 <=== 目前大数据量表格导入仍然有问题，可能是API或者服务器自身问题。 目前小数据量的表格可以方便的导入。
...
根据 Wolai 官方文档，API 调用存在以下限制：

### 调用限制
*   **批量获取接口**：一次最多获取 **200** 个记录。如想获取更多记录，请使用分页参数。
*   **创建接口**：一次最多创建 **20** 个记录。
*   **更新接口**：一次最多更新 **20** 个记录。
*   **删除接口**：每次只能删除一条记录，暂不支持批量删除。
*   **资源上传接口**：一次只能上传一个附件。单个附件大小最大不能超过 **1024M**。
*   **频率限制**：同一个用户 API 的请求频率上限为 **5次/秒**。

### 用量限制 (按版本)

| 版本 | 每小时使用限制 | 每月调用限制 | 上传图片/附件限制 |
| :--- | :--- | :--- | :--- |
| **个人免费版** | 10次 | 100次/月 | ≤5M/个 |
| **个人专业版** | 500次 | 10,000次/月 | ≤10M/张 (图片), ≤100M/个 (附件) |
| **家庭版** | 800次 | 20,000次/月 | ≤10M/张 (图片), ≤200M/个 (附件) |
| **小组版** | 1,000次 | 30,000次/月 | ≤10M/张 (图片), ≤400M/个 (附件) |
| **团队版** | 1,500次 | 60,000次/月 | ≤10M/张 (图片), ≤600M/个 (附件) |
| **企业版** | 3,000次 | 200,000次/月 | ≤10M/张 (图片), ≤1024M/个 (附件) |

*注意：单个数据表格最多支持 10,000 行，100 列记录，10 个视图。*
